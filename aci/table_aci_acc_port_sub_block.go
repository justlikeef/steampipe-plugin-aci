...
func tableACIAccSubPortBlock() *plugin.Table {
	return &plugin.Table{
		Name:        "aci_access_sub_port_block",
		Description: "ACI Access Subport Block",
		List: &plugin.ListConfig{
			Hydrate: listLeafAccSubPortBlock,
		...
		Columns: []*plugin.Column{
			...
			{
				Name:        "to_sub_port",
				Description: "(Optional) To sub port for object access sub port block. Allowed Values are between 1 to 64. Default Value is \"1\".",
				Type:        proto.ColumnType_STRING,
			},
			{
...
func AccSubPortBlockFromContainerList(cont *container.Container, ctx context.Context, d *plugin.QueryData) error {
			...
			if reltype == "infraHPortS" {
				...
				for j := 0; j < len(infraHPortsChildren); j++ {
					...
					for relchildtype, relchildattribs := range infraHPortsChildrenMap {
						...
						if relchildtype == "infraSubPortBlk" {
							recordobj := AccSubPortBlock{}
							...
							recordobj.FromSubPort = client.G(relchildattribs.S("attributes"), "fromSubPort")
							...
							recordobj.ToSubPort = client.G(relchildattribs.S("attributes"), "toSubPort")
...
//// LIST FUNCTION
func listLeafAccSubPortBlock(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	for i := 0; i < length; i++ {
		err = AccSubPortBlockFromContainerList(leafintprofcont.S("imdata").Index(i).S("infraAccPortP"), ctx, d)
	}
